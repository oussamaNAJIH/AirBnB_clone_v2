...
def do_deploy(archive_path):
    ...
    try:
        ...
        run('tar -xzf /tmp/{} -C {}{}/'.format(file_name, release_path, no_extension))
        # Remove the temporary archive file
        run('rm /tmp/{}'.format(file_name))
        # Move contents from web_static subdirectory to release directory
        run('mv {}/web_static/* {}/'.format(release_path + no_extension, release_path + no_extension))
        ...
        run('ln -s {}/ /data/web_static/current'.format(release_path + no_extension))
        return True
    except:
        return False
